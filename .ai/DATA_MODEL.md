...
| Identidad del CANAL (identificador de canal→documento, VERIFICADO por código) | ZANTIA — tabla `identidad_canal` (SQLite propio, `domains/health/identity_store.py`, recado 014, extendido a Telegram en 022) | Distinto del "titular"/"gestor"/"beneficiario" de más abajo: esos son conceptos de UNA conversación; `identidad_canal` es la asociación identificador-de-canal↔documento en sí, válida para TODAS las conversaciones futuras de ese identificador. La columna `telefono` (nombre heredado de cuando solo existía WhatsApp/Chatwoot) desde recado 022 también guarda `chat.id` de Telegram — mismo concepto ("identificador de canal"), sin necesidad de renombrar la columna ni el esquema. Fuente de verdad de "¿ya verificamos que este identificador de canal es dueño de este documento?" — nunca hrmm-backend (que no tiene noción de número de WhatsApp/chat de Telegram) ni `ConversationState` (que muere con la conversación) |
