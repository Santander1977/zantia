...
## Retención de identidad de canal: vencimiento automático + olvido a pedido (2026-09-03, recado 016)

Resuelve R-20 (política de retención de `identidad_canal`, dejada explícitamente PENDIENTE en 014) con dos decisiones de producto del usuario:

**Vencimiento automático a 180 días** — `domains/health/identity_store.py:RETENCION_IDENTIDAD_DIAS` (constante única, nombrada). `IdentidadCanal.vigente()` exige `estado == VERIFICADO` Y `verificado_en` dentro de esa ventana; la hidratación en `gateway.py:handle_inbound_message` usa `.vigente()` en vez de solo `estado == VERIFICADO`. Una fila vencida se trata EXACTAMENTE como si no existiera — sin ningún mensaje especial ("tu identidad venció"), el wizard completo de 012/014 se dispara de nuevo. Re-verificar actualiza `verificado_en` (UPSERT ya existente desde 014), nunca duplica la fila.

**Eliminación a pedido del paciente** — `HealthBrain` (`brain.py:_OLVIDAR`, frases como "olvida mi información"/"borra mis datos") detecta la intención en CUALQUIER etapa de una conversación ya abierta, con la misma prioridad que reprogramar/cancelar/confirmar (chequeada ANTES del resto del dispatch por etapa). Wizard de 2 pasos PROPIO (`confirmando_olvido` → `olvido_confirmado`), análogo en espíritu al de beneficiario (013) pero sin tocar ninguno de sus campos (requisito #5). El Brain solo PROPONE — el borrado real de la fila (`identity_store.eliminar`, nunca un cambio de estado) y el registro `IDENTIDAD_ELIMINADA_A_PEDIDO` en EventLog los ejecuta `gateway.py:_procesar_olvido_si_corresponde`, porque `identity_store` es un servicio de `HealthGateway`, no de `HealthAgentContext` (mismo criterio arquitectónico que la hidratación de identidad). Solo una aceptación clara confirma el borrado; cualquier otra respuesta restaura la etapa exacta en la que estaba el paciente, sin perder su lugar en la conversación.

**Bug real encontrado y corregido durante la implementación**: la rama de "no confirmó" proponía `proxima_accion_propuesta="preguntar_intencion"`, que el Core solo permite como transición válida desde ciertas fases (`state/machine.py:VALID_TRANSITIONS`) — usarla sin condición disparaba una escalación REAL por "error de transición" cuando el olvido se pedía a mitad de una etapa alcanzada vía `RECOPILACION_DE_DATOS` (ej. "esperando_seleccion"). Corregido a `"preguntar_dato_faltante"` (válida desde cualquier fase donde este wizard puede interrumpirse), mismo valor que ya usaba la rama de inicio del olvido.

**Alcance no cubierto, declarado explícitamente**: (a) el olvido solo se detecta dentro de una conversación YA abierta (`gateway.py:handle_inbound_message`, rama `contexto_existente`) — un primer mensaje de un teléfono sin conversación previa que diga "olvida mi información" se clasifica por `intent.py` como cualquier otra intención, sin llegar a `HealthBrain`; (b) `agent.py:handle_reminder_response` (respuesta a un recordatorio) no pasa por `gateway.py`, así que el olvido no se detecta ahí tampoco.

**Hallazgo no buscado**: `HrmmAppointmentService.book_appointment` nunca envía `correo` al crear una cita — documentado como R-21 (recado 015), sin relación directa con este cambio pero encontrado durante el trabajo de esta fase de identidad.

Tests: `tests/domains/health/test_identidad_olvido.py` (6 tests nuevos) + `test_identidad_persistente.py` (+6 tests de retención) + los 130 anteriores = 142/142 (+ 2 deshabilitados a propósito).
